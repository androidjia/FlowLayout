 ...
 * @Details: <心电图回看>
 */
public class EcgReView  extends View {
...
//    private float baseNumber;//基数
    ...
    public EcgReView(Context context) {
        this(context,null);
    }

    public EcgReView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }

    public EcgReView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
        ...
        if (mTouchEndX > mMarginLeft&& mTouchEndX < right) {
            if (mTouchY<=ecgBottom) {//移动心电图
                ...
                if (totalNum>0) {
                    ...
                    float span = lastPressLine/baseLineWidth;//每个时间段的长度
                    float lenTime = span*duration/1000;//当前时长


                    if (tempList != null && tempList.size()>0) {
                        drawBubbleTempAndHeartRate(canvas,"℃",lastPressLine,Color.parseColor("#67D267"),tempList,true);
                    }

                    if (heartRateList != null && heartRateList.size()>0) {
                        drawBubbleTempAndHeartRate(canvas,"bpm",lastPressLine,Color.parseColor("#F59C7A"),heartRateList,false);
                    }
...
//                Log.i("zero","===============x diff:"+mDiffX);
                ...
                break;
            case MotionEvent.ACTION_UP://滑动停止调用数据
                postInvalidate();
                break;
        }
